...
class TransactionActor extends Actor with LazyLogging with TransactionImplicit {
    ...
    case resp: BankTransactionResponse =>
      resp.account match {
        case Some(_) =>
          ...
          //TODO Check roar type and if rounding can be applied
          if (resp.transaction.amount < 0) {
            ...
            mortgageActor ! resp.copy(account = Some(account.copy(balance = account.balance + resp.transaction.amount)))
          } else {
        ...
        case Some(mortgage) =>
          saveTrans(resp.transaction) onComplete {
            case Success(_) =>
              ...
              val intAmount = math.ceil(resp.transaction.amount * -1)
              val extra = BigDecimal(intAmount + resp.transaction.amount).setScale(2, BigDecimal.RoundingMode.UP).toDouble
              val teniiTransaction = Transaction(resp.transaction.userId, "Tenii", "Debt-Sort-Code", "Debt-Acc-Num", resp.transaction.date, -extra)
              mortgageActor ! mortgage.copy(balance = mortgage.balance + teniiTransaction.amount)
  ...
  def updateBankBalance(trans: Transaction, bankAccount: BankAccount): Unit = bankAccountActor ! bankAccount.copy(balance = bankAccount.balance + trans.amount)
}
