...
import com.ogunleye.tenii.products.model.domain.{ BankTransactionResponse, MortgageBankTransactionResponse }
...
class MortgageActor extends Actor with LazyLogging with MortgageImplicit {
  ...
  override def receive: Receive = {
    ...
    case request: BankTransactionResponse =>
      val senderRef = sender()
      Future {
        connection.findByUserId(request.transaction.userId)
      } onComplete {
        case Success(result) => senderRef ! MortgageBankTransactionResponse(request.ref, request.transaction, request.account, result)
        case Failure(t) =>
          logger.error(s"Error retrieving mortgage for user: ${request.transaction.userId}", t)
          senderRef ! MortgageBankTransactionResponse(request.ref, request.transaction, request.account, None)
      }
